/**
 * This C program tracks an array of students using a struct. It also implements reading and writing files.
 *
...
void sort(struct studentRecord studentTemp);
...
int addSort(char* studentName_input, char* major_input, char* schoolYear_input, unsigned int studentID_input)
{
	...
	// add the student to the end of the list and initialize the students information
	strcpy(list[count].studentName, studentName_input);
	strcpy(list[count].major, major_input);
	// convert schoolYear_input string to enum type
	schoolYear sy = 0;       // declare a schoolYear type sy to store the enum
	...
	list[count].schoolYear = sy;
	list[count].studentID = studentID_input;

	// store the new student in studentTemp
	studentTemp = list[count];

	// increment the number of students currently stored in the list
	count++;
...
void sort(struct studentRecord studentTemp) {
	...
	for (int i = 0; i < count; i++) {
		if (strcmp(studentTemp.studentName, list[i].studentName) > 0) {
			pos = i + 1;
		}
	}
...
void load(char* fileName)
{
	...
	// Parse the list and read student record
	// Check if the file exists
	if (file != NULL) {
		fread(&count, sizeof(count), 1, file);		// store the number of students in the file
		for (i = 0; i < count; i++)
		{
			...
			fread(&list[i].schoolYear, sizeof(schoolYearValue), 1, file);

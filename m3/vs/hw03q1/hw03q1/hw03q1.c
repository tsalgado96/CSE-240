/**
 * This C program practices manipulating char arrays (strings) using pointer operations.
 *
...
void initializeStrings(char strings[NUM_STRINGS][STRING_LENGTH])
{
	...
	// enter code here
	int TOTAL_CELLS = NUM_STRINGS * STRING_LENGTH;
...
char* reverseOneString(char s[STRING_LENGTH])
{
	...
	// enter code here
	char *end = s;          // pointer to end of string
	end += strlen(s) - 1;
	...
	p = s; // return the pointer back to the beginning of the string
	return p;
}
...
int isPalindrome(char s[STRING_LENGTH])
{
	...
	// enter code here
	char *end = s;          
	end += strlen(s) - 1;       // pointer to end of string
	for (int i = 0; i < strlen(s) / 2; i++) {
		if (*p != *end) {
			return 0;
		}
